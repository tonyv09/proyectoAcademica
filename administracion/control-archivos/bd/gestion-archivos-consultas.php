...
//echo "GESTION ARCHIVOS CONSULTAS";
...
echo "PASS ".$_SESSION['clave_acceso'];*/
if($length<104857600 )
{
...
$resinfile=pg_query($conexion,"insert into schema_academica.ruta_archivo (ruta,nombre_archivo,pantalla_destino,descripcion,fecha_periodo,estado) values('$ruta_descargar','$nombreArchivo','$nombre_pantalla','$ubicacion','$fecha','$estado')")or die('2 No se ha podido insertar RUTA DE ARCHIVOS ERROR: ' . pg_last_error());
if ($resinfile) {
$accion="Insersion del archivo ".$nombreArchivo;
...
$accion="Eliminacion de archivo en ruta ".$fila->ruta;
$fecha=date('Y-m-d H:i:s');
pg_query($conexion,"INSERT INTO schema_academica.bitacora (usuario,accion,fecha) VALUES(current_user,'$accion','$fecha')")or die('2 Error en insersion '.pg_last_error());
}else
